...
public class MultipleInstrumentsCandleStrategy {

  private static final Logger logger = LoggerFactory.getLogger(MultipleInstrumentsCandleStrategy.class);

  private final MultipleInstrumentsCandleStrategyConfiguration configuration;
  private final ServiceStubFactory serviceFactory;
  private final MarketDataStreamManager streamManager;

  public MultipleInstrumentsCandleStrategy(
        ...
        .thenAcceptAsync(historicCandles -> {
          ...
          streamManager.subscribe(MarketDataRequest.newBuilder()
            .setSubscribeCandlesRequest(SubscribeCandlesRequest.newBuilder()
              ...
              .build())
            .build());
          logger.info("Subscribed for candles for instrument {}", entry.getKey());
  ...
  private void proceedNewCandle(CandleWrapper candle) {
    var foundInstrument = configuration.getInstruments().stream()
      .filter(instrument ->
