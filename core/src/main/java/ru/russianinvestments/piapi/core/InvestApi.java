...
import ru.russianinvestments.piapi.core.stream.MarketDataStreamService;
import ru.russianinvestments.piapi.core.stream.OperationsStreamService;
import ru.russianinvestments.piapi.core.stream.OrdersStreamService;
...
public class InvestApi {
  ...
  private static final String defaultAppName = "russianinvestments.invest-api-java-sdk";
  ...
  public static InvestApi createSandbox(@Nonnull String token) {
    var target = Optional.ofNullable(System.getenv("TINKOFF_INVEST_API_TARGET_SANDBOX"))
      .orElseGet(() -> props.getProperty("ru.russianinvestments.piapi.core.sandbox.target"));
  ...
  public static Channel defaultChannel(String token, String appName, String target) {
    ...
    try {
      var availableTimeOutValue = Optional.ofNullable(System.getenv("TINKOFF_INVEST_API_CONNECTION_TIMEOUT"))
        .orElseGet(() -> props.getProperty("ru.russianinvestments.piapi.core.connection-timeout"));
      connectionTimeout = Duration.parse(availableTimeOutValue);
    } catch (DateTimeParseException e) {
      connectionTimeout = Duration.parse(props.getProperty("ru.russianinvestments.piapi.core.connection-timeout"));
    }

    Duration requestTimeout;
    try {
      var availableTimeOutValue = Optional.ofNullable(System.getenv("TINKOFF_INVEST_API_REQUEST_TIMEOUT"))
        .orElseGet(() -> props.getProperty("ru.russianinvestments.piapi.core.request-timeout"));
      requestTimeout = Duration.parse(availableTimeOutValue);
    } catch (DateTimeParseException e) {
      requestTimeout = Duration.parse(props.getProperty("ru.russianinvestments.piapi.core.request-timeout"));
    }
  ...
  public static Channel defaultChannel(String token, String appName) {
    var target = Optional.ofNullable(System.getenv("TINKOFF_INVEST_API_TARGET"))
      .orElseGet(() -> props.getProperty("ru.russianinvestments.piapi.core.api.target"));
